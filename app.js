...
function renderProducts() {
  ...
  let products=state.db.products.filter(p=>[p.name,p.brand,p.location,p.category].join(' ').toLowerCase().includes(query));
...
function productCard(p) { const status=productStatus(p),exp=sortedExpirations(p).slice(0,4).map(e=>fmtDate(e.date)).join(', '),safePhoto=String(p.photo||'').startsWith('data:image/')?escapeHtml(p.photo):'';return `<article class="product-card"><div class="product-photo">${safePhoto?`<img src="${safePhoto}" alt="${escapeHtml(p.name)}">`:'🍎'}</div><div class="product-head"><div><h3>${escapeHtml(p.name)}</h3><p class="muted small">${escapeHtml(p.brand||'Sem marca')} · ${escapeHtml(p.category||'Outros')}</p></div><button class="icon-btn favorite-product" data-id="${escapeHtml(p.id)}">${p.favorite?'★':'☆'}</button></div><div><span class="badge ${status.cls}">${escapeHtml(status.label)}</span></div><p><strong>${productQty(p)}</strong> ${escapeHtml(p.unit||'un')} · <span class="muted">${escapeHtml(p.location||'Sem local')}</span></p><p class="muted small">Validades: ${exp||'Não informadas'}</p><div class="product-actions"><button class="soft-btn consume-product" data-id="${escapeHtml(p.id)}" ${productQty(p)===0?'disabled':''}>Consumir</button><button class="soft-btn edit-product" data-id="${escapeHtml(p.id)}">Editar</button><button class="soft-btn duplicate-product" data-id="${escapeHtml(p.id)}">Duplicar</button></div></article>`; }

function openProduct(id=null) { state.editingId=id;const p=id?state.db.products.find(x=>x.id===id):null;$('#productDialogTitle').textContent=p?'Editar produto':'Novo produto';$('#deleteProductBtn').classList.toggle('hidden',!p);$('#productId').value=p?.id||'';$('#name').value=p?.name||'';$('#category').value=p?.category||'Outros';$('#brand').value=p?.brand||'';$('#quantity').value=p?productQty(p):1;$('#unit').value=p?.unit||'un';$('#location').value=p?.location||'';$('#barcode').value=p?.barcode||'';$('#notes').value=p?.notes||'';$('#expirations').value=sortedExpirations(p||{}).map(e=>e.date).join('\n');$('#photo').value='';$('#productDialog').showModal(); }
...
async function saveProductForm(event) {
  event.preventDefault();
  try {
    const id=$('#productId').value||uid(),existing=state.db.products.find(p=>p.id===id),photo=await fileToBase64($('#photo').files[0]);
    const dates=$('#expirations').value.split(/\n|,/).map(v=>v.trim()).filter(Boolean);if(dates.some(date=>!parseDate(date)))throw new Error('Use datas no formato AAAA-MM-DD.');
    const consumed=(existing?.expirations||[]).filter(e=>e.consumed),active=dates.map(date=>{const prior=(existing?.expirations||[]).find(e=>!e.consumed&&e.date===date);return prior||{id:uid(),date,consumed:false};});
    const product={id,name:$('#name').value.trim(),category:$('#category').value,brand:$('#brand').value.trim(),quantity:Number($('#quantity').value),unit:$('#unit').value,location:$('#location').value.trim(),barcode:$('#barcode').value.trim(),notes:$('#notes').value.trim(),photo:photo||existing?.photo||'',expirations:[...consumed,...active],favorite:existing?.favorite||false,createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
    if(!product.name)throw new Error('Informe o nome do produto.');if(product.quantity<active.length)throw new Error('A quantidade não pode ser menor que o número de validades informadas.');
    if(existing)Object.assign(existing,product);else state.db.products.unshift(product);const history=historyEntry(product.name,productQty(product),existing?'Correção':'Entrada');state.db.history.unshift(history);$('#productDialog').close();await enqueue('saveProduct',{product,history});
  } catch(error) { toast(error.message); }
}
